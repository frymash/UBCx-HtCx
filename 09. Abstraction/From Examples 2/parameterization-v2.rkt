...
;; given a string and (list s0 s1...), return true if the string
...
(check-expect (contains? "McGill" (cons "McGill" empty)) true)
(check-expect (contains? "McGill" (cons "UBC" (cons "McGill" empty))) true)
(check-expect (contains? "Quebec" (cons "UBC" (cons "McGill" empty))) false)
...
(check-expect (map2 sub1 (list 1 2 3)) (list 0 1 2))
...
;; given a Boolean fn and (list n0 n1...), return (list na nb...nn)
;; where na nb...nn are numbers that cause the Boolean fn to return
;; true when used as input
